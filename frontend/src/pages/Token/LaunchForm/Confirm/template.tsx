...
import { useTeamAllocationTotalPercentage } from 'src/hooks/useLaunchForm'
...
export default function LaunchTemplate({ liquidityPrice, teamAllocationPrice }: LaunchTemplateProps) {
  // eth price
        ...
        <Row className={styles.amountRowContainer}>
          ...
          {liquidityPrice ? (
            <Row className={styles.amountContainer}>
              ...
              <Text.Body>{liquidityPrice ? `${formatCurrenyAmount(liquidityPrice, { fixed: 4 })} ETH` : '-'}</Text.Body>
            </Row>
          ) : (
